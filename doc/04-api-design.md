...
本文件描述当前 MVP 已实现或已预留的接口。未列入本文档的 OpenAI 扩展接口，如 Images、Audio、Files、Batches，属于后续版本范围。
...
| 429 | `quota_exceeded` | Key、项目或预算额度耗尽 |
| 429 | `budget_exceeded` | 预算策略触发拦截 |
...
| `stream` | boolean | 否 | 当前 MVP 按非流式处理 |
...
Provider 资源池是高级扩展能力，用于同一个 Provider 下管理多区域、多 Key 或多本地集群。MVP 主流程推荐直接用多个 Provider 表达多上游备份。
...
通用资源接口用于团队、额度、预算、成本中心、报表、通知渠道、安全策略等后台配置。资源类型通过 `{kind}` 区分。
...
| POST | `/api/admin/resources/invoices/{id}/confirm` | 确认内部账单 |
| POST | `/api/admin/resources/invoices/{id}/reject` | 驳回内部账单 |
...
| POST | `/api/admin/billing/generate` | 生成指定期间账单 |
| GET | `/api/admin/export/{kind}` | 导出 CSV |

常见导出 `kind`：`requests`、`usage`、`cost-centers`、`budgets`、`chargebacks`、`invoices`、`approvals`、`audit-events`、`alert-deliveries`。

#### 审计与告警

| Method | Path | 说明 |
| --- | --- | --- |
| GET | `/api/admin/audit/requests` | 请求审计列表 |
...
| 能力 | 计划说明 |
| --- | --- |
| Images API | 后续支持 `/v1/images/generations` 并纳入模型目录的 image 类模型 |
| Audio API | 后续支持语音转写、TTS、实时语音 |
| Files/Batches | 后续支持离线任务和批处理 |
| OpenAPI 导出 | 后续生成 `openapi.json`，可导入 Apifox、Swagger、Postman |
