...
| --- | --- |
| 管理员 | 全局 |
| 安全管理员 | 全局请求日志、安全策略、告警和通知 |
| 团队负责人 | 所属团队的用户、项目、Key、用量、成本和请求日志 |
| 普通用户 | 自己创建的 Key、自己的用量和自己的请求日志 |
...
当前安全策略以敏感内容脱敏、IP 白名单和错误透传模式为主，复杂 DLP 可作为增强能力。
...
- TokenHub 不鼓励也不支持非授权账号分发或 API 分发。
